...
import {
  ...
  Layers, Type, Tag, BrainCircuit, X
} from 'lucide-react';
...
const ProductFormPage = () => {
    ...
    const initData = async () => {
      try {
        ...
        const mockCategories = [
           { id: 1, name: 'AI & Chatbot' }, { id: 2, name: 'Giải trí' },
           { id: 3, name: 'Game' }, { id: 4, name: 'Phần mềm' }
        ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Lỗi: " + (error.response?.data?.message || "Kiểm tra lại thông tin."));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
            <button onClick={() => navigate('/admin/products')} className="p-2 bg-slate-800 rounded-lg hover:bg-slate-700 text-gray-400 hover:text-white"><ArrowLeft size={20}/></button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 space-y-4">
                <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Type size={18} className="text-blue-400"/> Thông tin cơ bản</h3>
                ...
                <div className="pt-2 border-t border-slate-800">
                    <div className="flex items-center gap-2 mb-2 text-white font-medium">
                        <Tag size={16} className="text-yellow-500"/> Từ khóa tìm kiếm (SEO & Search)
                    </div>
                    <TagInput 
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">Các từ đồng nghĩa, viết tắt, tiếng việt có dấu/không dấu mà người dùng hay tìm.</p>
                </div>
            </div>

            {/* 2. Dữ liệu AI (Metadata) */}
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 space-y-4">
                <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><BrainCircuit size={18} className="text-pink-500"/> Dữ liệu AI (Metadata)</h3>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 space-y-3">
                <h3 className="text-sm font-bold text-gray-400 uppercase mb-2 flex items-center gap-2"><ImageIcon size={16}/> Hình ảnh</h3>
                <input type="text" name="thumbnail" value={formData.thumbnail} onChange={handleChange} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-xs text-gray-300 focus:border-vtv-green outline-none" placeholder="Link ảnh..."/>
                <div className="aspect-video rounded-lg bg-slate-950 border border-slate-700 flex items-center justify-center overflow-hidden">
                    {formData.thumbnail ? <img src={formData.thumbnail} className="w-full h-full object-cover" onError={(e) => e.target.style.display='none'}/> : <span className="text-gray-600 text-xs">No Image</span>}
                </div>
            </div>
        </div>
