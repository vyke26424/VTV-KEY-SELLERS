...
import { Search, Plus, Filter, Trash2, Edit, Key, Package, X } from 'lucide-react';
...
const StockListPage = () => {
    ...
    const [pagination, setPagination] = useState({ page: 1, limit: 20, totalPage: 1, total: 0 });
    ...
    useEffect(() => { fetchStocks(1); }, [activeStatus]);
    ...
    return (
        <div className="space-y-6 pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Package className="text-vtv-green"/> Quản lý Kho Key</h1>
                    <p className="text-gray-400 text-sm mt-1">Tổng: {pagination.total} key trong hệ thống</p>
                </div>
                <button 
            ...
            <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex gap-4">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-400 text-sm font-bold">Lọc trạng thái:</span>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setActiveStatus('')} className={`px-3 py-1 text-xs font-bold rounded border ${activeStatus === '' ? 'bg-white text-black border-white' : 'bg-slate-800 text-gray-400 border-slate-700'}`}>Tất cả</button>
                    <button onClick={() => setActiveStatus('AVAILABLE')} className={`px-3 py-1 text-xs font-bold rounded border ${activeStatus === 'AVAILABLE' ? 'bg-green-500 text-black border-green-500' : 'bg-slate-800 text-gray-400 border-slate-700'}`}>Chưa bán</button>
                    <button onClick={() => setActiveStatus('SOLD')} className={`px-3 py-1 text-xs font-bold rounded border ${activeStatus === 'SOLD' ? 'bg-blue-500 text-white border-blue-500' : 'bg-slate-800 text-gray-400 border-slate-700'}`}>Đã bán</button>
                </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl">
                        ...
                        {loading ? (
                            ...
                            stocks.map(item => (
                                <tr key={item.id} className="hover:bg-slate-800/50 transition">
                                    <td className="p-4">
                                        <div className="font-bold text-white text-sm">{item.productName || 'Unknown Product'}</div>
                                        <div className="text-xs text-vtv-green mt-0.5">{item.variantName || 'Unknown Variant'}</div>
                                    </td>
                                    <td className="p-4 font-mono text-gray-400 text-sm tracking-widest">
                                        {item.credential}
                                    </td>
                                    <td className="p-4 text-center">
                                        <span className={`px-2 py-1 rounded text-[10px] font-bold border ${STATUS_COLORS[item.status]}`}>
                                            {item.status}
                                        </span>
                                    </td>
                                    <td className="p-4 text-sm text-gray-300">
                                        {item.status === 'SOLD' ? (
                                            <div className="text-xs">
                                                <span className="block text-white">Đơn: #{item.orderCode}</span>
                                            </div>
                            ...
                            <tr><td colSpan="5" className="p-8 text-center text-gray-500">Kho đang trống hoặc không tìm thấy.</td></tr>
                        )}
                    </tbody>
                </table>
                ...
                <div className="p-4 border-t border-slate-800 flex justify-center gap-2">
                    <button disabled={pagination.page===1} onClick={() => fetchStocks(pagination.page-1)} className="px-3 py-1 bg-slate-800 rounded disabled:opacity-50">Trước</button>
                    <span className="px-3 py-1 text-white font-bold">{pagination.page} / {pagination.totalPage}</span>
                    <button disabled={pagination.page===pagination.totalPage} onClick={() => fetchStocks(pagination.page+1)} className="px-3 py-1 bg-slate-800 rounded disabled:opacity-50">Sau</button>
                </div>
            </div>

            {/* --- MODAL NHẬP KHO (ImportModal) --- */}
            {showImportModal && (
                <ImportStockModal onClose={() => setShowImportModal(false)} onSuccess={() => {
                    fetchStocks(1);
...
const ImportStockModal = ({ onClose, onSuccess }) => {
    ...
    useEffect(() => {
        const fetchProds = async () => {
            ...
            const res = await axiosClient.get('/admin/product', { params: { limit: 100 } });
    ...
    const handleSubmit = async () => {
        if (!selectedVariantId || !rawKeys.trim()) return alert("Vui lòng chọn gói và nhập key!");
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-2xl shadow-2xl p-6">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2"><Plus size={20} className="text-vtv-green"/> Nhập kho hàng loạt</h3>
                ...
                <div className="space-y-4">
                    {/* Chọn Sản phẩm */}
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">1. Chọn Sản phẩm</label>
                        <select 
                    ...
                    {/* Chọn Variant */}
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">2. Chọn Gói (Variant)</label>
                        <select 
                            className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white outline-none focus:border-vtv-green"
                        ...
                        >
                            ...
                            {variants.map(v => <option key={v.id} value={v.id}>{v.name} - {v.price}đ</option>)}
                        </select>
                    </div>

                    {/* Nhập Key */}
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">3. Danh sách Key (Mỗi key 1 dòng)</label>
                        <textarea 
                            rows="6"
                        ...
                        ></textarea>
                        <p className="text-xs text-gray-500 mt-1 text-right">Đang nhập: {rawKeys.split('\n').filter(k=>k.trim()).length} key</p>
                    </div>
