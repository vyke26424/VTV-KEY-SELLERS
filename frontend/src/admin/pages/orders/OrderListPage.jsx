...
import { Search, Eye, X, Package, Calendar, User, CreditCard } from 'lucide-react';
...
const OrderListPage = () => {
    ...
    const handleUpdateStatus = async (newStatus) => {
        ...
        try {
            setIsUpdating(true);
            await axiosClient.patch(`/admin/orders/${selectedOrder.id}/status`, { status: newStatus });
    ...
    return (
        <div className="space-y-6 pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-white flex items-center gap-2"><CreditCard className="text-vtv-green"/> Quản lý Đơn hàng</h1>
            </div>
            ...
            <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                <div className="relative max-w-md">
                    ...
                    <input 
                        type="text" placeholder="Tìm theo mã đơn hoặc email khách..." 
                        value={searchTerm}
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl">
                        ...
                        {loading ? (
                            ...
                            orders.map(order => (
                                <tr key={order.id} className="hover:bg-slate-800/50 transition">
                                    <td className="p-4 font-mono font-bold text-white">{order.code}</td>
                                    <td className="p-4 text-sm text-gray-300">
                                        <div className="font-bold text-white">{order.user?.fullName || 'Guest'}</div>
                                        <div className="text-xs text-gray-500">{order.user?.email}</div>
                                    </td>
                                    <td className="p-4 font-bold text-vtv-green">{formatCurrency(order.totalAmount)}</td>
                                    <td className="p-4 text-xs text-gray-400">{formatDate(order.createdAt)}</td>
                                    <td className="p-4 text-center">
                                        <span className={`px-2 py-1 rounded text-[10px] font-bold border ${STATUS_COLORS[order.status] || 'text-gray-500'}`}>
                                            {order.status}
                                        </span>
                                    </td>
                ...
                <div className="p-4 border-t border-slate-800 flex justify-center gap-2">
                    <button disabled={pagination.page===1} onClick={() => fetchOrders(pagination.page-1)} className="px-3 py-1 bg-slate-800 rounded disabled:opacity-50">Trước</button>
                    <span className="px-3 py-1 text-white font-bold">{pagination.page} / {pagination.totalPage}</span>
                    <button disabled={pagination.page===pagination.totalPage} onClick={() => fetchOrders(pagination.page+1)} className="px-3 py-1 bg-slate-800 rounded disabled:opacity-50">Sau</button>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-slate-900 border border-slate-700 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
                        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950">
                            <h3 className="text-xl font-bold text-white">Chi tiết đơn {selectedOrder.code}</h3>
                        ...
                        <div className="p-6 overflow-y-auto space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4 bg-slate-800/50 p-4 rounded-xl border border-slate-800">
                                <div>
                                    ...
                                    <div className="text-xs text-gray-400 mt-1 ml-6">{selectedOrder.user?.email}</div>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-gray-500 uppercase font-bold">Ngày đặt</p>
                                    <p className="text-white mt-1 text-sm">{formatDate(selectedOrder.createdAt)}</p>
                                    <div className="mt-2">
                                        <span className={`px-2 py-1 rounded text-xs font-bold border ${STATUS_COLORS[selectedOrder.status]}`}>
                            ...
                            {/* Danh sách sản phẩm */}
                            <div>
                                <h4 className="text-sm font-bold text-gray-400 uppercase mb-3">Sản phẩm đã mua</h4>
                                        ...
                                        <div key={item.id} className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex gap-3">
                                            <div className="w-12 h-12 bg-slate-800 rounded flex items-center justify-center shrink-0">
                                                {item.variant?.product?.thumbnail ? (
                                                    <img src={item.variant.product.thumbnail} className="w-full h-full object-cover rounded"/>
                                                ) : <Package size={20} className="text-gray-500"/>}
                                            </div>
                                            <div className="flex-1">
                                                <div className="text-white font-bold text-sm">{item.variant?.product?.name}</div>
                                                <div className="text-gray-400 text-xs">{item.variant?.name} x {item.quantity}</div>
                                                <div className="text-vtv-green font-mono text-sm mt-1">{formatCurrency(item.price)}</div>
                                            </div>
                            ...
                            <div className="flex justify-between items-center pt-4 border-t border-slate-800">
                                ...
                                <span className="text-2xl font-bold text-vtv-green">{formatCurrency(selectedOrder.totalAmount)}</span>
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-slate-950 border-t border-slate-800 flex justify-end gap-3">
                            ...
                            <button onClick={() => setSelectedOrder(null)} className="px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700">Đóng</button>
                        </div>
                    </div>
                </div>
            )}
