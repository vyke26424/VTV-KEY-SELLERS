          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <h1 className="text-3xl font-bold text-white flex items-center gap-3">
                <Search className="text-vtv-green" size={32} />"{query}"
              </h1>
            </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="bg-slate-800/50 rounded-xl h-80 animate-pulse"
              ></div>
            ))}
          </div>
        ) : products.length > 0 ? (
          ...
          <div className="text-center py-20 bg-slate-800/30 rounded-2xl border border-dashed border-slate-700">
            ...
            <p className="text-gray-400 mb-6">
              Hãy thử tìm với từ khóa khác hoặc xem các danh mục phổ biến.
            </p>
            <Link
              to="/"
              className="bg-vtv-green text-black font-bold px-6 py-3 rounded-full hover:bg-green-400 transition"
            >
              Về trang chủ
            </Link>
          </div>
