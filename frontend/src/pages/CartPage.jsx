...
const CartPage = () => {
  ...
  const totalAmount = cart.reduce((total, item) => {
    // Giá item * số lượng
    return total + (parseFloat(item.selectedVariant?.price || item.price) * item.quantity);
  }, 0);
    ...
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        <div className="bg-slate-800/50 rounded-2xl p-10 max-w-2xl mx-auto border border-dashed border-slate-700">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Giỏ hàng đang trống</h2>
          <p className="text-gray-400 mb-6">Bạn chưa thêm sản phẩm nào. Hãy dạo một vòng xem sao nhé!</p>
          <Link to="/" className="bg-vtv-green text-black font-bold px-6 py-3 rounded-full hover:bg-green-400 transition inline-block">
            Quay lại mua sắm
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8 min-h-screen">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-white uppercase">Giỏ hàng của bạn ({cart.length})</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {cart.map((item) => (
            <div key={item.cartItemId} className="bg-vtv-card border border-slate-700 rounded-xl p-4 flex flex-col md:flex-row gap-4 items-center">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="text-gray-400 text-sm">
                  Đơn giá: {formatCurrency(item.selectedVariant?.price || item.price)}
                </div>
              </div>
          ...
          <button onClick={clearCart} className="text-gray-500 hover:text-white text-sm underline mt-2">
            Xóa tất cả sản phẩm
          </button>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-vtv-card border border-slate-700 rounded-xl p-6 sticky top-24 shadow-xl">
            <h3 className="text-xl font-bold text-white mb-6 border-b border-slate-700 pb-4">Tóm tắt đơn hàng</h3>
            ...
            <div className="flex items-start gap-3 text-xs text-gray-500 bg-slate-800 p-3 rounded-lg">
               ...
               <p>Đơn hàng được bảo vệ bởi chính sách hoàn tiền 100% nếu sản phẩm lỗi. Thanh toán an toàn và bảo mật.</p>
            </div>
          </div>
        </div>
